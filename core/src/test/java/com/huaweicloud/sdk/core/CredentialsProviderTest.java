...
public class CredentialsProviderTest {
    ...
    @Test
    public void testCredentialProviderChain() {
        String file = Paths.get("src", "test", "resources", "TestCredentials")
                .toAbsolutePath().toString();
        environmentVariables.set("HUAWEICLOUD_SDK_CREDENTIALS_FILE", file);
        BasicCredentials credentials = (BasicCredentials) CredentialProviderChain
                .getBasicCredentialProviderChain().getCredentials();

        Assert.assertEquals("your_ak", credentials.getAk());
        Assert.assertEquals("your_sk", credentials.getSk());
        Assert.assertEquals("your_project_id", credentials.getProjectId());

        environmentVariables.clear("HUAWEICLOUD_SDK_CREDENTIALS_FILE");
    }
}
