...
public class FormDataClientTest {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(FormDataClientTest.class);

    private static final int FILE_SIZE = 1024 * 1024 * 5;

    HcClient hcClient;

    HttpRequestDef<HttpRequestDefTest.TestFormDataRequest, HttpRequestDefTest.TestFormDataResponse> testFormDataDef
            = HttpRequestDefTest.buildTestFormDataRequestDef();
    ...
    public void testUploadDownloadRequest() {
        ...
        HttpRequestDefTest.TestFormDataRequest request = new HttpRequestDefTest.TestFormDataRequest();
        request.withBody(body -> {
            body.setUuid("hello");
            body.withUploadFile(byteArrayInputStream, "text.txt", "application/octet-stream");
        });

        HttpRequestDefTest.TestFormDataResponse response = hcClient.syncInvokeHttp(request, testFormDataDef);
