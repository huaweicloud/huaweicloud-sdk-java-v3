...
public class RomaClient {
    ...
    /** 绑定域名 用户自定义的域名，需要CNAME到API分组的子域名上才能生效。 每个API分组下最多可绑定5个域名。绑定域名后，用户可通过自定义域名调用API。
     *
    ...
    /** 绑定签名密钥 签名密钥创建后，需要绑定到API才能生效。 将签名密钥绑定到API后，则ROMA Connect APIC请求后端服务时就会使用这个签名密钥进行加密签名，后端服务可以校验这个签名来验证请求来源。
    ...
    /** 实例配置特性 为实例配置需要的特性。 支持配置的特性列表及特性配置请参考“附录 &gt; 实例支持的APIC特性”
     *
     ...
     * 租户创建一个签名密钥，并将签名密钥与API进行绑定，则ROMA Connect
     * APIC在请求这个API时，就会使用绑定的签名密钥对请求参数进行数据加密，生成签名。当租户的后端服务收到请求时，可以校验这个签名，如果签名校验不通过，则该请求不是ROMA Connect
     * APIC发出的请求，租户可以拒绝这个请求，从而保证API的安全性，避免API被未知来源的请求攻击。
     *
    ...
    /** 删除客户端配额 删除客户端配额。删除客户端配额时，同时删除客户端配额和客户端应用的关联关系
     *
    ...
    /** 删除流控策略 删除指定的流控策略。当该流控策略绑定了API时，需要先解除流控策略与API的所有绑定关系后再删除。
     *
    ...
    /** 删除签名密钥 删除指定的签名密钥。签名密钥绑定了API时无法删除，需要先解除与API的绑定关系后删除。
     *
    ...
    /** 查询客户端配额可绑定的客户端应用列表 查询客户端配额可绑定的客户端应用列表。支持按客户端应用名称模糊搜索
     *
    ...
    /** 查询客户端配额已绑定的客户端应用列表 查询客户端配额已绑定的客户端应用列表。支持按客户端应用名称模糊匹配
     *
    ...
    /** 获取客户端配额列表 获取客户端配额列表。支持根据名称模糊查询
     *
    ...
    /** 查看实例特性列表 查看实例特性列表。注意：实例不支持以下特性的需要联系技术支持升级实例版本。 支持配置的特性列表及特性配置请参考“附录 &gt; 实例支持的APIC特性”
     *
    ...
    /** API指标统计值查询-最近一段时间 根据API的id和最近的一段时间查询API被调用的次数，统计周期为1分钟。查询范围一小时以内，一分钟一个样本，其样本值为一分钟内的累计值。 &gt;
    ...
    /** 查询后端API部署历史 在某个实例中查询后端API的部署记录。
     *
    ...
    /** 查询后端API测试结果 在某个实例中查询后端API的测试结果。
     *
    ...
    /** 查询客户端应用关联的应用配额 查看指定客户端应用关联的应用配额。
     *
    ...
    /** 修改变量 修改环境变量。环境变量引用位置为api的后端服务地址时，修改对应环境变量会将使用该变量的所有api重新发布。
     *
    ...
    /** 批量发布或下线API 将多个API发布到一个指定的环境，或将多个API从指定的环境下线。 注意：当action &#x3D; online时，接口返回的响应中publish_id，version_id，
     * publish_time字段才有含义。
     *
    ...
    /** 校验API定义 校验API定义。校验API的路径或名称是否已存在
     *
    ...
    /** 删除API分组 删除指定的API分组。 分组下存在API时分组无法删除，需要删除所有分组下的API后，再删除分组。
    ...
    /** 查看APP下路径冲突的api列表 查询指定APP下路径冲突的api列表。
     *
    ...
    /** 设置用户成员 - 设置应用的用户成员，为空数组时会清空已有应用成员列表 - 设置动作为全量更新非增量更新，应用的成员列表都会替换为当次请求的应用成员列表
     *
    ...
    /** 查询候选用户成员 查询应用的候选用户成员列表,会过滤掉异常状态用户
     *
    ...
    /** 查询应用列表 查询应用列表，支持条件查询，所有条件是并且的关系
     *
    ...
    /** 校验字典是否存在 校验指定条件的字典是否存在，支持字典名称和字典编码
     *
    ...
    /** 导出API 导出分组下API的定义信息，导出文件内容符合swagger标准规范。
     *
    ...
    /** 导出自定义后端API 导出自定义后端API，导出文件内容符合swagger标准规范。
     *
    ...
    /** 导入API 导入API。导入文件内容需要符合swagger标准规范，自定义扩展字段请参考用户指南的“附录：前端API的Swagger扩展定义”章节。
     *
    ...
    /** 导入自定义后端API 导入自定义后端API。导入文件内容需要符合swagger标准规范，自定义扩展字段请参考用户指南的“附录：后端API的Swagger扩展定义”章节
     *
