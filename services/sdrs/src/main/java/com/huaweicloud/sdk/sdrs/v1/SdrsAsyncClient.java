...
public class SdrsAsyncClient {
    ...
    /** 添加保护实例标签 一个保护实例上最多有10个标签。此接口为幂等接口：创建时，如果创建的标签已经存在（key相同），则覆盖。
     *
    ...
    /** 批量添加保护实例标签 为指定保护实例批量添加或删除标签。一个资源上最多有10个标签。 此接口为幂等接口： 创建时如果请求体中存在重复key则报错。
    ...
    /** 批量创建保护实例 典型场景：没有特殊操作场景 接口功能：批量创建保护实例。保护实例创建完成后，系统默认容灾站点云服务器名称与生产站点云服务器名称相同，但ID不同。
     *
    ...
    /** 批量删除保护实例 典型场景：没有特殊操作场景 接口功能：批量删除保护实例。
     *
    ...
    /** 批量删除保护实例标签 为指定保护实例批量删除标签。一个资源上最多有10个标签。 此接口为幂等接口：
    ...
    /** 创建保护实例 创建保护实例。保护实例创建完成后，系统默认容灾站点云服务器名称与生产站点云服务器名称相同，但ID不同。如果需要修改云服务器名称，请在保护实例详情页面单击云服务器名称，进入云服务器详情页面进行修改
     *
    ...
    /** 创建保护组 创建保护组。 说明： 本接口为异步接口，调用成功只是表示请求下发，创建结果需要通过“查询job状态”接口获取
     *
    ...
    /** 删除所有保护组失败任务 删除所有保护组层级的失败任务，创建、删除保护组失败等。
     *
    ...
    /** 删除容灾演练 删除指定的容灾演练。删除后： 容灾演练服务器、容灾演练服务器上挂载的磁盘和网卡将被一并删除。 演练VPC、演练VPC的子网不会被删除。您可以继续使用该VPC创建其他云服务器。
     *
    ...
    /** 删除保护实例标签 幂等接口：删除时，不对标签字符集做校验，调用接口前必须要做encodeURI，服务端需要对接口URI做decodeURI。 说明:请自行选择工具执行URI编码。
    ...
    /** 删除指定保护组内的所有失败任务 删除指定保护组内的所有失败任务，创建保护实例失败、创建复制对失败、删除保护实例失败、删除复制对失败等。
     *
    ...
    /** 查询双活域 查询双活域。双活域由本端存储设备、远端存储设备组成，通过双活域，应用服务器可以实现跨站点的数据访问。
     *
    ...
    /** 查询容灾演练列表 查询指定保护组下的所有容灾演练列表，当未指定保护组时查询当前租户下的所有容灾演练列表。
     *
    ...
    /** 查询失败任务列表 查询所有保护组失败任务列表或者指定保护组下的所有失败任务列表。
     *
    ...
    /** 查询保护实例项目标签 查询租户在指定Project中保护实例的所有资源标签集合。
     *
    ...
    /** 查询复制对列表 查询指定保护组下的所有复制对列表，如果不给定指定保护组则查询当前租户下的所有复制对列表。
     *
    ...
    /** 查询资源的RPO超标趋势记录列表 查询当前租户大屏显示中，资源的RPO超标趋势记录列表。
     *
    ...
    /** 保护实例变更规格 变更指定保护实例中弹性云服务器的规格，包括：同时变更生产站点云服务器和容灾站点云服务器的规格。 仅变更生产站点云服务器的规格，容灾站点云服务器规格不变。
    ...
    /** 查询单个保护实例详情 查询单个保护实例的详细信息，如名称、ID等。
     *
    ...
    /** 保护组故障切换 当保护组的生产站点发生故障时，将保护组的生产站点切到当前的容灾站点，即另一端AZ，启用当前容灾站点的云硬盘以及云服务器等资源。
    ...
    /** 保护组开启保护/重保护 对某一个保护组的“开启保护”或“重保护”操作。
     *
    ...
    /** 查询job状态 查询job的执行状态。 对于创建保护组、删除保护组、创建保护实例、删除保护实例、创建复制对、删除复制对等异步API，命令下发后，会返回job_id，通过job_id可以查询任务的执行状态。
     *
