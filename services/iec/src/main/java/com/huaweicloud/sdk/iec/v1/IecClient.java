...
public class IecClient {
    ...
    /** 切换操作系统 切换边缘实例操作系统，支持边缘实例创建成功后，保持ip、数据盘不变的情况下重装操作系统。 调用该接口后，系统将卸载系统盘，然后使用新镜像重新创建系统盘，并挂载至实例，实现切换操作系统功能。
     *
    ...
    /** 创建部署计划 为方便您的统一管理，以及跨边缘站点管理资源，IEC基于业务场景角度，定义了边缘业务。
     ...
     * 创建一个部署计划并执行，即可创建一个边缘业务。 - 边缘业务下实例分布取决于部署计划的实例分布与调度策略。 - 边缘业务下实例名称、规格、镜像等参数取决于部署计划配置计算实例字段。
     *
    ...
    /** 创建和导入密钥 创建SSH密钥，或把公钥导入系统，生成密钥对。 创建SSH密钥成功后，请把响应数据中的私钥内容保存到本地文件，用户使用该私钥登录边缘实例。为保证边缘实例安全，私钥数据只能读取一次，请妥善保管。
     *
    ...
    /** 删除虚拟私有云 根据虚拟机私有云的ID，删除对应的虚拟私有云。
     *
    ...
    /** 查询镜像列表 根据不同条件查询镜像列表，例: - 查询已注册的私有镜像列表: visibility&#x3D;private - 公共镜像: visibility&#x3D;public
     *
    ...
    /** 查询安全组规则详情 根据安全组规则的ID，获取安全组规则的详细信息。
     *
    ...
    /** 查询虚拟私有云详情 根据虚拟私有云ID，获取虚拟私有云的详情。
     *
    ...
    /** 删除弹性公网IP 根据弹性公网IP的ID，删除对应的弹性公网IP。
     *
    ...
    /** 更新弹性公网IP 更新弹性公网IP的信息，主要用于解绑和绑定EIP和VIP之间的关系。
     *
