package com.huaweicloud.sdk.identitycenter.v1.model;

import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;

import java.util.Objects;
import java.util.function.Consumer;

/**
 * UpdateApplicationInstanceResponseSchemaConfiguration的请求体
 */
public class UpdateApplicationInstanceResponseSchemaConfigurationReqBody {

    @JsonInclude(JsonInclude.Include.NON_NULL)
    @JsonProperty(value = "response_schema_config")

    private ResponseSchemaConfigDto responseSchemaConfig;

    public UpdateApplicationInstanceResponseSchemaConfigurationReqBody withResponseSchemaConfig(
        ResponseSchemaConfigDto responseSchemaConfig) {
        this.responseSchemaConfig = responseSchemaConfig;
        return this;
    }

    public UpdateApplicationInstanceResponseSchemaConfigurationReqBody withResponseSchemaConfig(
        Consumer<ResponseSchemaConfigDto> responseSchemaConfigSetter) {
        if (this.responseSchemaConfig == null) {
            this.responseSchemaConfig = new ResponseSchemaConfigDto();
            responseSchemaConfigSetter.accept(this.responseSchemaConfig);
        }

        return this;
    }

    /**
     * Get responseSchemaConfig
     * @return responseSchemaConfig
     */
    public ResponseSchemaConfigDto getResponseSchemaConfig() {
        return responseSchemaConfig;
    }

    public void setResponseSchemaConfig(ResponseSchemaConfigDto responseSchemaConfig) {
        this.responseSchemaConfig = responseSchemaConfig;
    }

    @Override
    public boolean equals(java.lang.Object obj) {
        if (this == obj) {
            return true;
        }
        if (obj == null || getClass() != obj.getClass()) {
            return false;
        }
        UpdateApplicationInstanceResponseSchemaConfigurationReqBody that =
            (UpdateApplicationInstanceResponseSchemaConfigurationReqBody) obj;
        return Objects.equals(this.responseSchemaConfig, that.responseSchemaConfig);
    }

    @Override
    public int hashCode() {
        return Objects.hash(responseSchemaConfig);
    }

    @Override
    public String toString() {
        StringBuilder sb = new StringBuilder();
        sb.append("class UpdateApplicationInstanceResponseSchemaConfigurationReqBody {\n");
        sb.append("    responseSchemaConfig: ").append(toIndentedString(responseSchemaConfig)).append("\n");
        sb.append("}");
        return sb.toString();
    }

    /**
     * Convert the given object to string with each line indented by 4 spaces
     * (except the first line).
     */
    private String toIndentedString(java.lang.Object o) {
        if (o == null) {
            return "null";
        }
        return o.toString().replace("\n", "\n    ");
    }

}
