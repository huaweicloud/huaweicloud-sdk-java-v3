...
public class ListClustersRequest {
    ...
    /** 可以通过集群的标签来搜索指定标签的集群，当指定多个tag进行查询时，标签之间是与的关系。 tags参数的格式为tags=k1\\*v1,k2\\*v2,k3\\*v3
    ...
    /** 根据集群状态查询集群列表。 starting：查询启动中的集群列表。 running：查询运行中的集群列表。 terminated：查询已删除的集群列表。 failed：查询失败的集群列表。
     * abnormal：查询异常的集群列表。 terminating：查询删除中的集群列表。 frozen：查询已冻结的集群列表。 scaling-out：查询扩容中的集群列表。 scaling-in：查询缩容中的集群列表。
     * 
     * @return clusterState */
    ...
    /** 通过企业项目ID来搜索指定项目的集群。 该参数默认设置为0，表示为default企业项目。
