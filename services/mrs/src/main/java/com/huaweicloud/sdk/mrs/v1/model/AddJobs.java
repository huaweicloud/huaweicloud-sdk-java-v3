...
public class AddJobs {
    ...
    /** 作业类型码。 - 1：MapReduce - 2：Spark - 3：Hive Script - 4：HiveSQL（当前不支持） - 5：DistCp，导入、导出数据，（当前不支持）。 - 6：Spark Script -
    ...
    /** 执行程序Jar包或sql文件地址，需要满足如下要求： - 最多为1023字符，不能包含;|&>,<'$特殊字符，且不可为空或全空格。 - 文件可存储于HDFS或者OBS中，不同的文件系统对应的路径存在差异。 -
     * OBS：以“s3a://”开头。不支持KMS加密的文件或程序。 - HDFS：以“/”开头。 - Spark Script需要以“.sql”结尾，MapReduce和Spark
     * Jar需要以“.jar”结尾，sql和jar不区分大小写。
     * 
     * @return jarPath */
    ...
    /** 数据输入地址。 文件可存储于HDFS或者OBS中，不同的文件系统对应的路径存在差异。 - OBS：以“s3a://”开头。不支持KMS加密的文件或程序。 - HDFS：以“/”开头。
    ...
    /** 数据输出地址。 文件可存储于HDFS或者OBS中，不同的文件系统对应的路径存在差异。 - OBS：以“s3a://”开头。 - HDFS：以“/”开头。 如果该路径不存在，系统会自动创建。
     * 最多为1023字符，不能包含;|&>'<$特殊字符，可为空。
     * 
     * @return output */
    ...
    /** 作业日志存储地址，该日志信息记录作业运行状态。 文件可存储于HDFS或者OBS中，不同的文件系统对应的路径存在差异。 - OBS：以“s3a://”开头。 - HDFS：以“/”开头。
     * 最多为1023字符，不能包含;|&>'<$特殊字符，可为空。
     * 
     * @return jobLog */
    ...
    /** sql程序路径，仅Spark Script和Hive Script作业需要使用此参数。需要满足如下要求： - 最多为1023字符，不能包含;|&><'$特殊字符，且不可为空或全空格。 -
     * 文件可存储于HDFS或者OBS中，不同的文件系统对应的路径存在差异。 - OBS：以“s3a://”开头。不支持KMS加密的文件或程序。 - HDFS：以“/”开头。 - 需要以“.sql”结尾，sql不区分大小写。
     * 
    ...
    public boolean equals(java.lang.Object o) {
        ...
        return Objects.equals(this.jobType, addJobs.jobType) && Objects.equals(this.jobName, addJobs.jobName)
            && Objects.equals(this.jarPath, addJobs.jarPath) && Objects.equals(this.arguments, addJobs.arguments)
            && Objects.equals(this.input, addJobs.input) && Objects.equals(this.output, addJobs.output)
            && Objects.equals(this.jobLog, addJobs.jobLog)
            && Objects.equals(this.hiveScriptPath, addJobs.hiveScriptPath) && Objects.equals(this.hql, addJobs.hql)
