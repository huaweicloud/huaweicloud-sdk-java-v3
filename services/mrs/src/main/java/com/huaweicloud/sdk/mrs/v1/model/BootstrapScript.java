...
public class BootstrapScript {
     ...
     * continue：继续执行后续脚本。 - errorout：终止操作。 */
    ...
    /** 枚举值： - BEFORE_COMPONENT_FIRST_START: 组件首次启动后 - AFTER_COMPONENT_FIRST_START: 组件首次启动前 - BEFORE_SCALE_IN: 缩容前 -
     * AFTER_SCALE_IN: 缩容后 - BEFORE_SCALE_OUT: 扩容前 - AFTER_SCALE_OUT: 扩容后 */
    public static final class ActionStagesEnum {

        /** Enum BEFORE_COMPONENT_FIRST_START for value: "BEFORE_COMPONENT_FIRST_START" */
        public static final ActionStagesEnum BEFORE_COMPONENT_FIRST_START =
            new ActionStagesEnum("BEFORE_COMPONENT_FIRST_START");

        /** Enum AFTER_COMPONENT_FIRST_START for value: "AFTER_COMPONENT_FIRST_START" */
        public static final ActionStagesEnum AFTER_COMPONENT_FIRST_START =
            new ActionStagesEnum("AFTER_COMPONENT_FIRST_START");
     ...
     * OBS桶的路径：直接手动输入脚本路径。例如输入MRS提供的公共样例脚本路径。示例：s3a://bootstrap/presto/presto-install.sh，其中安装dualroles时，presto-install.sh脚本参数为dualroles,
     * 安装worker时，presto-install.sh脚本参数为worker。根据Presto使用习惯，建议您在Active Master节点上安装dualroles，在Core节点上安装worker。 -
    ...
    /** 选择引导操作脚本执行的时间。 - BEFORE_COMPONENT_FIRST_START: 组件首次启动后 - AFTER_COMPONENT_FIRST_START: 组件首次启动前 - BEFORE_SCALE_IN:
     * 缩容前 - AFTER_SCALE_IN: 缩容后 - BEFORE_SCALE_OUT: 扩容前 - AFTER_SCALE_OUT: 扩容后
     * 
     * @return actionStages */
    ...
    public boolean equals(java.lang.Object o) {
        ...
        return Objects.equals(this.name, bootstrapScript.name) && Objects.equals(this.uri, bootstrapScript.uri)
