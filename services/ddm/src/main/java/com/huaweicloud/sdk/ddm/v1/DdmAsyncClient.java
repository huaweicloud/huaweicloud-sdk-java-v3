...
public class DdmAsyncClient {
    ...
    /** 购买DDM实例 创建一个DDM实例。 DDM运行于虚拟私有云。申请DDM实例前，需保证有可用的虚拟私有云，并且已配置好子网与安全组。
     *
    ...
    /** 创建DDM帐号 DDM帐号用于连接和管理逻辑库。一个DDM实例最多能创建100个DDM帐号，一个DDM帐号可以关联多个逻辑库。
     *
    ...
    /** 删除DDM帐号 删除指定的DDM实例帐号，如果帐号关联了逻辑库，则对应的关联关系也会删除。
     *
    ...
    /** 查询创建逻辑库可选取的数据库实例列表 查询创建逻辑库可选取的数据库实例列表。
     *
    ...
    /** 慢日志监控 查询指定时间段内在DDM实例上执行过的慢sql相关信息。
     *
    ...
    /** DDM表数据重载 DDM实例跨region容灾场景下，针对目标DDM实例实现表数据reload，使数据同步。
     *
    ...
    /** 同步数据库实例信息 同步当前DDM实例已关的所有数据库实例配置信息。
     *
    ...
    /** 修改DDM已关联的数据库实例的读策略 修改DDM已关联的数据库实例的读策略。
     *
    ...
    /** 修改DDM帐号 修改现有DDM帐号的权限或者与逻辑库的管理关系。
     *
