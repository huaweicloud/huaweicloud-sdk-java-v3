...
/**
 * **参数解释**：慢启动信息。开启慢启动后，将会在设定的时间段（duration）内对新添加到后端服务器组的后端服务器进行预热，转发到该服务器的请求数量线性增加。  **约束限制**： - 当后端服务器组的协议为HTTP/HTTPS/GRPC[/GRPCS](tag:not_open)时有效，其他协议传入该字段将报错。 - 慢启动与会话保持不能同时开启。若都开启则会导致会话保持失效。  网关型LB，不支持该特性，请勿使用。 [荷兰region不支持该字段，请勿使用。](tag:dt)
 */
