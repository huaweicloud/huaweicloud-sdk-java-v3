...
public class ElbClient {
    ...
    /** 创建后端云服务器组 创建后端云服务器组。将多个后端云服务器添加到后端云服务器组中后，请求会在后端云服务器间按后端云服务器组的负载均衡算法和后端云服务器的权重来做请求分发。
     *
    ...
    /** 创建白名单 创建白名单，控制监听器的访问权限。若开启了白名单功能，只有白名单中放通的IP可以访问该监听器的后端服务。
     *
    ...
    /** 删除监听器 根据指定ID删除监听器。提供级联删除选项，当选择级联删除时，删除和负载均衡器关联的转发规则、转发策略、白名单、标签等。
     *
    ...
    /** 删除负载均衡 根据指定ID删除负载均衡器。提供级联删除选项，当选择级联删除时，删除和负载均衡器关联的监听器、后端云服务器组、后端云服务器、健康检查、转发策略、转发规则、白名单、标签等
     *
    ...
    /** 查询所有监听器的标签列表 查询指定项目下所有监听器的标签列表
     *
    ...
    /** 查询监听器列表 查询监听器列表。支持过滤查询和分页查询。可以通过监听器ID、协议类型、监听端口号、关联的后端云服务器的IP等查询监听器。
     *
    ...
    /** 查询所有负载均衡器的标签列表 查询指定项目下所有负载均衡器的标签列表
     *
    ...
    /** 查询后端云服务器列表 添加属于某个后端云服务器组的后端云服务器。
     *
    ...
    /** 查询转发规则详情 根据指定ID查询某转发策略下关联的转发规则详情。
     *
    ...
    /** 查询负载均衡状态树 查询负载均衡器状态树。可通过该接口查询负载均衡器关联的监听器、后端云服务器组、后端云服务器、健康检查、转发策略、转发规则的主要信息，了解负载均衡器下资源的拓扑情况。
     *
    ...
    /** 更新白名单 更新白名单。可以打开或关闭白名单，或更新访问控制的IP。
     *
    ...
    /** 创建SSL证书 创建SSL证书。将监听器和SSL证书绑定后，可以通过负载均衡器实现服务端认证，后端服务器只要提供HTTP服务就能实现安全可靠的连接。
     *
