...
public class CcmClient {
    ...
    /** 创建委托 用户给PCA创建OBS委托授权，用于访问OBS桶，更新吊销列表。 &gt; 用户所使用账号token需要具备安全管理员（secu_admin）权限。
     *
    ...
    /** 通过CSR签发证书 通过CSR签发证书。功能约束如下： - 1、当前默认参数如下： - （1）CA 默认参数： - **keyUsage**: digitalSignature, keyCertSign,
     * cRLSign，优先采用CSR中的参数； - **SignatureHashAlgorithm**: SHA384； - **PathLength**：0 （可自定义）。 - （2）私有证书： - **keyUsage**:
     * digitalSignature keyAgreement，优先采用CSR中的参数； - **SignatureHashAlgorithm**: SHA384； -
     * 2、当传入的type为**INTERMEDIATE_CA**时，创建出的从属CA证书，有以下限制： - （1）不占用CA配额。在查询CA列表时，不会返回该证书； - （2）只支持通过以下两个接口获取其信息： - GET
     * /v1/private-certificate-authorities/{ca_id} 获取证书详情 - POST /v1/private-certificate-authorities/{ca_id}/export 导出证书
     * - （3）本接口返回的**certificate_id**即代表从属CA的**ca_id**； - （4）无法用于签发证书，密钥在用户侧。 - 3、当type为**ENTITY_CERT**时，创建出的私有证书，有以下特点：
     * - （1）占用私有证书配额。在查询私有证书列表时，会返回该证书； - （2）除了导出时不包含密钥信息（密钥在用户端），其余用法与其它私有证书一致。 &gt;
    ...
    /** 导出证书 导出证书。 - 选择是否压缩时，分以下两种情况： - is_compressed为true时，返回文件压缩包，命名为：证书名称_type字段小写字母.zip，如”test_apache.zip“。 - type
     * &#x3D;
     * \&quot;**APACHE**\&quot;时，压缩包中包含三个文件：**server.key**（密钥文件，内容为PEM格式）、**chain.crt**（证书链，内容为PEM格式）、**server.crt**（证书，内容为PEM格式）；
     * - type &#x3D;
     * \&quot;**IIS**\&quot;时，压缩包中包含两个文件：**keystorePass.txt**（keystore口令）、**server.pfx**（PFX证书，证书与证书链包含在同一个文件）； - type
     * &#x3D; \&quot;**NGINX**\&quot;时，压缩包中包含两个文件：**server.key**（密钥文件，内容为PEM格式）、**server.crt**（内容为PEM格式，证书与证书链包含在同一个文件）；
     * - type &#x3D;
     * \&quot;**TOMCAT**\&quot;时，压缩包中包含两个文件：**keystorePass.txt**（keystore口令）、**server.jks**（JKX证书，证书与证书链包含在同一个文件）； -
     * type &#x3D;
     * \&quot;**OTHER**\&quot;时，压缩包中包含三个文件：**server.key**（密钥文件，内容为PEM格式）、**chain.pem**（证书链）、**server.pem**（证书）。 -
     * is_compressed为false时，返回json格式，返回的具体参数如下： - type &#x3D;
     * \&quot;**APACHE**\&quot;或\&quot;**NGINX**\&quot;或\&quot;**OTHER**\&quot;时，返回参数如下： - **certificate**（证书内容，PEM格式）；
     * - **certificate_chain**（证书链，PEM格式）； - **private_key**（证书私钥，PEM格式）； - type &#x3D;
     * \&quot;**IIS**\&quot;或\&quot;**TOMCAT**\&quot;时，暂时未定义。 &gt; 只有当证书状态为“已签发”时，可进行导出操作。
     *
    ...
    /** 查询OBS桶列表 查询OBS桶列表。 &gt; 只有用户创建了委托授权，方可使用此接口。创建委托授权参见本文档：**证书吊销处理&gt;创建委托**。
     *
    ...
    /** 吊销证书 吊销证书。 &gt; 注：当不想填写吊销理由时，请求body体请置为\&quot;**{}**\&quot;，否则将会报错。
     *
    ...
    /** 查看是否具有委托权限 查看是否具有委托权限。 &gt; 用户所使用账号token需要具备安全管理员（secu_admin）权限。
     *
    ...
    /** 创建CA 创建CA，分以下三种情况： - 创建根CA，根据参数介绍中，填写必选值； - 创建从属CA，并需要直接激活该证书，根据参数介绍中，填写必选值； -
    ...
    /** 删除CA 计划删除CA。计划多少天后删除CA证书，可设置7天～30天内删除。 &gt; 只有当证书状态为”待激活“或”已禁用“状态时，才可删除。”待激活“状态下，将会立即删除证书，不支持延迟删除。
     *
    ...
    /** 禁用CA 禁用CA。 &gt; 只有当证书处于\&quot;已激活\&quot;或\&quot;已过期\&quot;状态时，可进行禁用操作。
     *
    ...
    /** 启用CA 启用CA。 &gt; 注：只有当证书处于\&quot;已禁用\&quot;状态时，可进行启用操作。
     *
    ...
    /** 导出CA证书 导出CA证书。 &gt; 注：只有当证书处于\&quot;已激活\&quot;或\&quot;已过期\&quot;时，可进行导出操作。
     *
     ...
     *
     * @param ExportCertificateAuthorityCertificateRequest 请求对象
     * @return SyncInvoker<ExportCertificateAuthorityCertificateRequest,
    ...
    /** 导出CA的证书签名请求（CSR） 导出CA的证书签名请求。 &gt; 只有当CA处于\&quot;待激活\&quot;状态时，可导出证书签名请求。
     *
    ...
    /** 导入CA证书 导入CA证书，使用本接口需要满足以下条件： - （1）证书为“待激活”状态的从属CA； - （2）导入的证书体必须满足以下条件： - a、该证书被签发时的证书签名请求必须是从PCA系统中导出； -
     * b、其证书链虽然允许不上传，但后期若想要导出完整的证书链，应导入完整的证书链； - c、证书体与证书链必须为PEM编码。
     *
     ...
     *
     * @param ImportCertificateAuthorityCertificateRequest 请求对象
     * @return SyncInvoker<ImportCertificateAuthorityCertificateRequest,
    ...
    /** 激活CA 激活CA。 &gt; 只有当证书处于\&quot;待激活\&quot;状态时，可进行激活操作。
     *
    ...
    /** 恢复CA 恢复CA，将处于“计划删除”状态的CA证书，重新恢复为“已禁用”状态。 &gt; 注：只有处于“计划删除”状态的CA证书，才可进行恢复操作。
     *
