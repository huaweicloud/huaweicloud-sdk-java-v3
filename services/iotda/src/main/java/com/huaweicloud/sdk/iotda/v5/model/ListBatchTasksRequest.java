...
public class ListBatchTasksRequest {
    ...
    /** **参数说明**：资源空间ID。此参数为非必选参数，存在多资源空间的用户需要使用该接口时，可以携带该参数查询指定资源空间下的任务列表，不携带该参数则会查询该用户下所有任务列表。
     ...
     * createAsyncCommands: 批量创建异步命令任务 - updateDeviceShadows：批量配置设备影子任务
     * 
     * @return taskType */
    ...
    /** **参数说明**：批量任务的状态，可选参数。 **取值范围**： - Initializing: 初始化中。 - Waitting: 等待中。 - Processing: 执行中。 - Success: 成功。 -
     * Fail: 失败。 - PartialSuccess: 部分成功。 - Stopped: 停止。
     * 
     * @return status */
    ...
    /** **参数说明**：分页查询时每页显示的记录数。 **取值范围**：1-50的整数，默认值为10。 minimum: 1 maximum: 50
     * 
     * @return limit */
