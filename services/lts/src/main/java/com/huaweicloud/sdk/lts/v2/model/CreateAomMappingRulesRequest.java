package com.huaweicloud.sdk.lts.v2.model;

import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.dataformat.xml.annotation.JacksonXmlProperty;

import java.util.Objects;
import java.util.function.Consumer;

/**
 * Request Object
 */
public class CreateAomMappingRulesRequest {

    @JsonInclude(JsonInclude.Include.NON_NULL)
    @JsonProperty(value = "isBatch")

    @JacksonXmlProperty(localName = "isBatch")

    private Boolean isBatch;

    @JsonInclude(JsonInclude.Include.NON_NULL)
    @JsonProperty(value = "body")

    @JacksonXmlProperty(localName = "body")

    private AomMappingRequestInfo body;

    public CreateAomMappingRulesRequest withIsBatch(Boolean isBatch) {
        this.isBatch = isBatch;
        return this;
    }

    /**
     * 是否开启自动映射
     * @return isBatch
     */
    public Boolean getIsBatch() {
        return isBatch;
    }

    public void setIsBatch(Boolean isBatch) {
        this.isBatch = isBatch;
    }

    public CreateAomMappingRulesRequest withBody(AomMappingRequestInfo body) {
        this.body = body;
        return this;
    }

    public CreateAomMappingRulesRequest withBody(Consumer<AomMappingRequestInfo> bodySetter) {
        if (this.body == null) {
            this.body = new AomMappingRequestInfo();
            bodySetter.accept(this.body);
        }

        return this;
    }

    /**
     * Get body
     * @return body
     */
    public AomMappingRequestInfo getBody() {
        return body;
    }

    public void setBody(AomMappingRequestInfo body) {
        this.body = body;
    }

    @Override
    public boolean equals(java.lang.Object o) {
        if (this == o) {
            return true;
        }
        if (o == null || getClass() != o.getClass()) {
            return false;
        }
        CreateAomMappingRulesRequest createAomMappingRulesRequest = (CreateAomMappingRulesRequest) o;
        return Objects.equals(this.isBatch, createAomMappingRulesRequest.isBatch)
            && Objects.equals(this.body, createAomMappingRulesRequest.body);
    }

    @Override
    public int hashCode() {
        return Objects.hash(isBatch, body);
    }

    @Override
    public String toString() {
        StringBuilder sb = new StringBuilder();
        sb.append("class CreateAomMappingRulesRequest {\n");
        sb.append("    isBatch: ").append(toIndentedString(isBatch)).append("\n");
        sb.append("    body: ").append(toIndentedString(body)).append("\n");
        sb.append("}");
        return sb.toString();
    }

    /**
     * Convert the given object to string with each line indented by 4 spaces
     * (except the first line).
     */
    private String toIndentedString(java.lang.Object o) {
        if (o == null) {
            return "null";
        }
        return o.toString().replace("\n", "\n    ");
    }

}
