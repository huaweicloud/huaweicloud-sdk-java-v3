...
public class KeystoneListProjectsRequest {
    ...
    /** 如果查询自己创建的项目，则此处应填为所属区域的项目ID。 如果查询的是系统内置项目，如cn-north-4，则此处应填为账号ID。
     * 获取项目ID和账号ID，请参见：[获取账号、IAM用户、项目、用户组、委托的名称和ID](https://support.huaweicloud.com/api-iam/iam_17_0002.html)。
     * 
     * @return parentId */
    ...
    /** 分页查询时数据的页数，查询值最小为1。需要与per_page同时存在。 minimum: 1
     * 
     * @return page */
    ...
    /** 分页查询时每页的数据个数，取值范围为[1,5000]。需要与page同时存在。 minimum: 1 maximum: 5000
     * 
     * @return perPage */
