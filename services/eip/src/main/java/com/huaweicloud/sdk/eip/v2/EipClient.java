...
public class EipClient {
    ...
    /** 批量创建弹性公网IP资源标签 为指定的弹性公网IP资源实例批量添加标签。
     *
    ...
    /** 批量删除弹性公网IP资源标签 为指定的弹性公网IP资源实例批量删除标签。
     *
    ...
    /** 申请弹性公网IP 申请弹性公网IP，支持IPv4和IPv6。 弹性公网IP（Elastic
     * IP）提供独立的公网IP资源，包括公网IP地址与公网出口带宽服务。可以与弹性云服务器、裸金属服务器、虚拟IP、弹性负载均衡、NAT网关等资源灵活地绑定及解绑。拥有多种灵活的计费方式，可以满足各种业务场景的需要。
     *
    ...
    /** 创建弹性公网IP资源标签 给指定弹性IP资源实例增加标签信息。
     *
    ...
    /** 删除弹性公网IP 删除弹性公网IP,绑定状态eip不允许直接删除。
     *
    ...
    /** 删除弹性公网IP的标签 删除指定弹性公网IP的标签信息。其中project_id是项目ID，publicip_id 是要操作的弹性公网IP的id。key是要删除标签的键。
     *
    ...
    /** 查询租户的弹性公网IP标签 查询租户在指定区域和实例类型的所有标签集合。
     *
    ...
    /** 更新弹性公网IP 更新弹性公网IP，将弹性公网IP跟一个网卡绑定或者解绑定，转换IP地址版本类型。
     *
    ...
    /** 创建浮动IP 创建浮动IP的外部网络UUID，请使用GET /v2.0/networks?router:external&#x3D;True或neutron net-external-list方式获取。
     *
    ...
    /** 查询浮动IP列表 查询提交请求的租户有权限操作的所有浮动IP地址。
     *
    ...
    /** 查询浮动IP 查询浮动IP详情，包括浮动IP状态，浮动IP所属路由器ID，浮动IP的外部网络ID等等。
     *
    ...
    /** 更新浮动IP 更新浮动IP。 更新时需在URL中给出浮动IP地址的ID。 port_id 为空，则表示浮动IP从端口解绑。
     *
