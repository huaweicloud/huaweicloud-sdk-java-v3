package com.huaweicloud.sdk.gsl.v3.model;

import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.dataformat.xml.annotation.JacksonXmlProperty;

import java.util.Objects;

/**
 * AddOrModifyAttributeReq
 */
public class AddOrModifyAttributeReq {

    @JsonInclude(JsonInclude.Include.NON_NULL)
    @JsonProperty(value = "cust_attr_name")

    @JacksonXmlProperty(localName = "cust_attr_name")

    private String custAttrName;

    public AddOrModifyAttributeReq withCustAttrName(String custAttrName) {
        this.custAttrName = custAttrName;
        return this;
    }

    /**
     * 自定义属性名称
     * @return custAttrName
     */
    public String getCustAttrName() {
        return custAttrName;
    }

    public void setCustAttrName(String custAttrName) {
        this.custAttrName = custAttrName;
    }

    @Override
    public boolean equals(java.lang.Object o) {
        if (this == o) {
            return true;
        }
        if (o == null || getClass() != o.getClass()) {
            return false;
        }
        AddOrModifyAttributeReq addOrModifyAttributeReq = (AddOrModifyAttributeReq) o;
        return Objects.equals(this.custAttrName, addOrModifyAttributeReq.custAttrName);
    }

    @Override
    public int hashCode() {
        return Objects.hash(custAttrName);
    }

    @Override
    public String toString() {
        StringBuilder sb = new StringBuilder();
        sb.append("class AddOrModifyAttributeReq {\n");
        sb.append("    custAttrName: ").append(toIndentedString(custAttrName)).append("\n");
        sb.append("}");
        return sb.toString();
    }

    /**
     * Convert the given object to string with each line indented by 4 spaces
     * (except the first line).
     */
    private String toIndentedString(java.lang.Object o) {
        if (o == null) {
            return "null";
        }
        return o.toString().replace("\n", "\n    ");
    }

}
