...
public class ListPublicZonesRequest {
    ...
    /** 待查询的zone的类型。 取值范围：public、private 如果为空，表示查询公网类型的zone。 如果为public，表示查询公网类型的zone。 如果为private，表示查询内网类型的zone。
     * 搜索模式默认为模糊搜索。 默认值为空。
     * 
     * @return type */
    ...
    /** 资源标签。 取值格式：key1,value1|key2,value2 多个标签之间用\"|\"分开，每个标签的键值用英文逗号\",\"相隔。 多个标签之间为“与”的关系。 关于资源标签，请参见添加资源标签。
     * 搜索模式为精确搜索。如果资源标签值value是以*开头时，则按照*后面的值全模糊匹配。 默认值为空。
     * 
     * @return tags */
