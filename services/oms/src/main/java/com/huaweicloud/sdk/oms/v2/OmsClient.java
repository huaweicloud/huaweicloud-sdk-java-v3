...
public class OmsClient {
    ...
    /** 创建同步事件 源端有对象需要进行同步时，调用该接口创建一个同步事件，系统将根据同步事件中包含的对象名称进行同步
     *
    ...
    /** 创建迁移任务 创建迁移任务，创建成功后，任务会被自动启动，不需要额外调用启动任务命令。
     *
    ...
    /** 删除迁移任务 调用该接口删除迁移任务。 正在运行的任务不允许删除，如果删除会返回失败；若要删除，请先行暂停任务。
     *
    ...
    /** 查询API版本信息列表 查询对象存储迁移服务的API版本信息。
     *
    ...
    /** 查询指定API版本信息 查询对象存储迁移服务指定API版本信息。
     *
